...
experiment_name = "CatBoost_Energy_Forecasting"
...
def optimize_and_train(data, group, target, covs, n_splits=5, n_trials=100):
    with mlflow.start_run(run_name=f"CatBoost_Optuna_{group}", nested=True) as run:
        ...
        mlflow.catboost.log_model(best_model,
                                 f"catboost_model_{group}",
